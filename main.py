...
def send_discord_webhook(games_list):
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={'Content-Type': 'application/json', 'User-Agent': 'SteamFreebieBot/1.0'}
        )
...
def check_deals():
    ...
    # We use a custom User-Agent so Reddit doesn't block the request
    ...
    for entry in feed.entries:
        link = entry.link
        # Ensure the link points directly to Steam
    ...
    if found_new_games:
        ...
        # Save newly emailed games to history to prevent duplicate alerts
